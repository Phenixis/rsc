...
use common::{write_silence_wav, write_sine_wav};
...
async fn wait_for<T>(
    ...
    mut pick: impl FnMut(&PlayerEvent) -> Option<T>,
) -> T {
...
async fn replacing_a_track_reports_stop_then_eof() {
    ...
    write_silence_wav(&long, 30.0);
    write_silence_wav(&short, 0.3);
    let (player, mut rx) = spawn().await;
...
fn position(event: &PlayerEvent) -> Option<f64> {
    match event {
...
/// Real-time playback (`--ao=null` keeps the clock): a paused player must stop advancing,
/// and resume from where it stopped.
#[tokio::test]
async fn pause_really_stops_the_playback_clock() {
    require_mpv!();
    let dir = tempfile::tempdir().unwrap();
    let wav = dir.path().join("tone.wav");
    write_sine_wav(&wav, 30.0, 440.0);
    let (player, mut rx) = spawn().await;

    player.load(&target(&wav)).await.unwrap();
    // The clock reading we start from: it must come from the event we just waited for,
    // not from a later one that may or may not arrive before mpv confirms the pause.
    let mut paused_at = wait_for(&mut rx, |e| position(e).filter(|p| *p > 0.2)).await;

    player.set_pause(true).await.unwrap();
    // Positions still in flight before mpv confirms the pause are fine; keep the last.
    wait_for(&mut rx, |e| {
        if let Some(p) = position(e) {
            paused_at = p;
        }
        (*e == PlayerEvent::Paused(true)).then_some(())
    })
    .await;

    // Half a second later the clock must not have moved (unpaused, it would be at +0.5 s).
    let mut furthest = paused_at;
    let _ = timeout(Duration::from_millis(600), async {
        while let Some(event) = rx.recv().await {
            if let Some(p) = position(&event) {
                furthest = furthest.max(p);
            }
        }
    })
    .await;
    assert!(
        furthest - paused_at < 0.25,
        "kept playing while paused: {paused_at} -> {furthest}"
    );

    player.set_pause(false).await.unwrap();
    wait_for(&mut rx, |e| position(e).filter(|p| *p > paused_at + 0.4)).await;
    player.shutdown().await;
}
